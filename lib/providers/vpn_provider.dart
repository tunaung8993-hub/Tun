...
class VpnProvider extends ChangeNotifier {
  // Your GitHub Gist Raw Link for remote server updates
  static const String remoteServerUrl = 'https://gist.githubusercontent.com/tunaung8993-hub/9cb7b34a166b0b5f9cdaab6ed97bed11/raw/servers.json';
  ...
  VpnProvider() {
    _servers = getDefaultServers(); // Initial load from local
    _selectedServer = _servers.isNotEmpty ? _servers.first : null;
    _initV2Ray();
    _fetchRealIp();
    refreshServers(); // Fetch from remote
  }

  Future<void> refreshServers() async {
    try {
      final response = await http.get(Uri.parse(remoteServerUrl)).timeout(const Duration(seconds: 10));
      ...
      // Keep using local servers if remote fails
    }
  }
